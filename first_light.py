...
(function cascadeRows() {
  ...
  const reveal = (el, n) => {
    ...
    el.classList.remove('will-rise');
    el.classList.add('is-in');
    io.unobserve(el);
  };
  // Hide only what is below the fold right now, and only after we know the
  // observer exists to bring it back.
  const fold = window.innerHeight;
  rows.forEach(r => {
    if (r.getBoundingClientRect().top > fold) r.classList.add('will-rise');
  });
...
/* Nothing is hidden by default. JS marks the rows below the fold, and only
   those. If the script never runs, or throws halfway, every row is readable,
   because a row must not depend on an animation firing in order to be seen. */
.map-row.will-rise { opacity: 0; transform: translateY(4px); }
.map-row.is-in {
